...
# Auto-generated script to mount and open USB drive /dev/sdc
echo "Attempting to mount /dev/sdc..."
...
if command -v udisksctl >/dev/null 2>&1; then
    ...
    PARTITIONS=$(ls /dev/sdc[0-9]* 2>/dev/null)
    ...
    if [ $MOUNTED -eq 0 ]; then
        if output=$(udisksctl mount -b "/dev/sdc" 2>&1); then
            echo "$output"
            MOUNT_POINT=$(echo "$output" | grep -o "at [^ ]*$" | cut -d' ' -f2)
            if [ -n "$MOUNT_POINT" ]; then
                nemo  "$MOUNT_POINT" >/dev/null 2>&1 &
    ...
    if [ $MOUNTED -eq 0 ]; then
        notify-send "USB Mount Error" "Failed to mount /dev/sdc or any of its partitions."
    fi
else
